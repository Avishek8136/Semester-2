function Regression_Newtonsmethod_one()
    syms x;
    f = x^2 -4; % Corrected the function definition
    W = NewtonsMethod(f, x);
...
function PlotMyModelAndData(f, x, W)
    ...
    y_values = double(subs(2*x - 4, x, x_values)); % Convert symbolic to numeric
    
    % Plot the function
    plot(x_values, y_values, 'b-', 'LineWidth', 2);
    xlabel('x');
    ylabel('f(x)');
    title('Plot of f(x) = x^2 - 4'); % Corrected the title
    grid on;
...
function W = NewtonsMethod(f, x)
    ...
    while condition
        n = f / diff(f);
        x_new = x_old - double(subs(n, x, x_old)); % Evaluate the iteration
